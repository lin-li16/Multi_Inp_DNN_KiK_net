...
def main():
    ...
    parser.add_argument('--path', type=str, default='inp_arg3_model', help='Parent file path of the dataset and the results')
    ...
    parser.add_argument('--datapre', type=str, default=None, help='DataProcesser file')
    ...
    if args.datapre is not None:
        with open(os.path.join(model_path, args.datapre), 'rb') as file:
            ...
            data_set.batch = args.batch
            data_set.train_loader = torch.utils.data.DataLoader(data_set.train_dataset, args.batch)
    else:
    ...
    if args.model == 'Basic_3inp':
        Net = Basic_3inp(kernel_size=args.kernel, drop_ratio=args.dropout)
    elif args.model == 'Basic_2inp':
        Net = Basic_2inp(kernel_size=args.kernel, drop_ratio=args.dropout)
    elif args.model == 'Basic_2inp_1':
        Net = Basic_2inp_1(kernel_size=args.kernel, drop_ratio=args.dropout)
    elif args.model == 'Basic_1inp':
        Net = Basic_1inp(kernel_size=args.kernel, drop_ratio=args.dropout)
    ...
    postprocess = PostProcess(slvr.valid_best_model, data_set, x_out=freq)
